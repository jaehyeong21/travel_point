...
public class  DesHome {
    ...
    public static String parseDescriptionFromResponse(String responseData) {
                ...
                if (itemsNode != null) {
                    ...
                    if (itemNode != null && itemNode.isArray()) { // item이 배열일 경우에만 반복
                        for (JsonNode item : itemNode) { // 각 item을 반복하면서 description 값을 가져옴
                            JsonNode overviewNode = item.get("overview");
    ...
    public static String parseHomepageFromResponse(String responseData) {
        ObjectMapper objectMapper = new ObjectMapper();
        try {
            // JSON 문자열을 JsonNode 객체로 파싱
            JsonNode rootNode = objectMapper.readTree(responseData);
                ...
                if (itemsNode != null) {
                    ...
                    if (itemNode != null && itemNode.isArray()) { // item이 배열일 경우에만 반복
                        for (JsonNode item : itemNode) { // 각 item을 반복하면서 homepage 값을 가져옴
                            JsonNode homepageNode = item.get("homepage");
